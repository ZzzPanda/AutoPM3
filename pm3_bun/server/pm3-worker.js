...
async function parsePdfWithMineru(buffer, fileName, mimeType) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`MinerU request failed (${response.status}): ${detail.slice(0, 1000)}`);
  }
  const raw = await response.json();
  const result = pickMineruResult(raw, fileName);
  if (!result || typeof result !== 'object') throw new Error('MinerU response did not contain parse results');
...
function findBestChunkIds(chunks, needles, limit = 3) {
  ...
  return normalizedChunks
    .map((chunk) => ({ id: chunk.id, score: tokens.reduce((total, token) => total + (chunk.text.includes(token) ? 1 : 0), 0) }))
...
function attachFullTextChunks(parsedResult, text) {
  ...
  const pm3Rows = parsedResult.pm3Rows.map((row) => {
    ...
    for (const [field, value] of Object.entries(row)) {
      ...
      fieldSources[field] = [...new Set(findBestChunkIds(fullChunks, [value, row.sourceIndex, row.rawTableRow], 3))].filter(Boolean);
      fieldHighlights[field] = evidenceTermsFor(value);
    }
  ...
  return { ...parsedResult, candidates, pm3Rows, chunks: fullChunks, documentMarkdown: text };
}
...
输出 JSON schema:
{
  "candidates": [{"variant":"候选 DNA/HGVS/论文位点表示；没有则 not stated","protein":"蛋白变化；没有则 not stated","gene":"基因；没有则 not stated","patients":"关联患者数/病例描述；没有则 not stated","context":"关联合子状态/上下文","partner":"第二等位基因/trans/cis partner；没有则 not stated","source":"chunk-1"}],
  "pm3Rows": [{"pmid":"PMID；没有则 not stated","title":"标题；没有则 not stated","authors":"作者；没有则 not stated","patientCount":"患者数","patientAge":"患者年龄","clinicalDisease":"患者临床疾病","clinicalPhenotype":"患者临床表型","familyInfo":"家系情况","pathogenicity":"致病性（作者结论是否致病/疑似致病/不致病可能有其它位点）","zygosity":"关联合子状态","transVariant":"反式(trans)位点","cisVariant":"顺式(cis)位点","sourceIndex":"来源索引（表编号+行号/段落/页码等可回溯位置）","rawTableRow":"表格原始行内容，保留原文","literatureBackground":"文献背景(是什么研究，最终什么结论)","summary":"总结：该变异已在[至少]XX名患有[疾病/表型]的个体中被检测到...","extractionProcess":"AI提取过程：说明如何定位和判断","source":"chunk-1"}],
  "chunks": [{"id":"chunk-1","title":"来源标题","reason":"为什么作为证据","quote":"可回溯的原文短摘录"}],
...
async function callOpenAiCompatible({ modelSettings, fileName, text }) {
  if (!modelSettings) throw new Error('Missing model settings: please set API URL, model name and API key');
  const response = await fetch(`${normalizeApiUrl(modelSettings.apiUrl)}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: 'system', content: 'You extract biomedical PM3 evidence and return strict JSON only.' },
        { role: 'user', content: buildPm3ExtractionPrompt({ fileName, text }) },
      ],
      temperature: 0,
  ...
  return ParsedResultSchema.parse(JSON.parse(extractJsonObject(content)));
}

async function updateStatus(status, parsedResult = undefined, errorMessage = null) {
  ...
  await sql`
    update autopm3_documents
    set status = ${status}, parsed_result = ${sql.json(parsedResult)}, error_message = ${errorMessage}, updated_at = now()
    where id = ${workerData.id}
  `;
}
...
async function run() {
  ...
  const buffer = await readUploadedFile(workerData.s3Key);
  const text = await extractText(buffer, workerData.mimeType, workerData.fileName);
  if (!text.trim()) throw new Error('No extractable text found in uploaded file');
  const extractedResult = workerData.parsedResult
    ? ParsedResultSchema.parse(workerData.parsedResult)
    : await callOpenAiCompatible({ modelSettings: workerData.modelSettings, fileName: workerData.fileName, text });
  const parsedResult = attachFullTextChunks(extractedResult, text);
  await updateStatus('parsed', parsedResult);
}
...
try {
  ...
  await updateStatus('failed', undefined, message);
