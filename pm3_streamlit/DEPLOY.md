# AutoPM3 Streamlit 部署与本地测试指南

> 本文档只讲 `pm3_streamlit/` 这一套(Python + Streamlit + Docker)。仓库根目录的 `README.md` 介绍两个子项目的总体关系;Bun/Node 那一套(Fastify + Vue)看 `pm3_bun/README.md`。

本指南假设你已经 `cd` 进 `pm3_streamlit/` 后再执行所有命令(除少数从仓库根调用的命令会显式说明)。

入口为 `app/main.py`(DeepSeek 页面)和 `app/pages/` 下的其它页面。下面分别说明：
...
> 整个仓库把 Python 套(`pm3_streamlit/`)和 Bun/Node 套(`pm3_bun/`)拆成两个独立子项目,根目录只保留 docs / benchmarks / license 等共享资产。本指南只涉及 `pm3_streamlit/`。
...
│   ├── xml_papers/            # download_papers.py 的输出(gitignored)
...
- **真实 `secrets.toml` 必须在 `pm3_streamlit/.streamlit/secrets.toml`**(Streamlit 默认搜索入口脚本所在目录),不能放到 `config/`。
- **`.dockerignore` 必须在 `pm3_streamlit/`**(Docker 工具链约束,build context 根),不要移到仓库根。
...
```bash
# 模板在 config/,但真实的 secrets.toml 必须放在 .streamlit/ 下
...
```bash
# 一定要在 pm3_streamlit/ 下,这样 streamlit 才能从 .streamlit/secrets.toml 读凭据
...
- **找不到 `data/protein.txt`**:CWD 不在 `pm3_streamlit/`。`app/core/query.py` 用 `__file__` 反推项目根,如果从别的目录跑,路径就解析错了。
...
`docker-compose.yml` 用 `context: ..` 和 `dockerfile: docker/Dockerfile`,所以 build context 是 `pm3_streamlit/`。需要在 `pm3_streamlit/` 下跑:
...
> 想从仓库根直接跑也可以:把 `pm3_streamlit/` 视作 compose 项目根即可,例如 `docker compose -f pm3_streamlit/docker/docker-compose.yml -p autopm3 up -d`。
...
`build.sh` 自解析 `SCRIPT_DIR` 和 `PROJECT_ROOT`,所以**在 `pm3_streamlit/` 下调用即可**。`PROJECT_ROOT` 会自动等于 `pm3_streamlit/`,Docker build 也能正确找到 `docker/Dockerfile` 和 `data/protein.txt`。

> 如果你希望从仓库根调用,可以加一行:`cd pm3_streamlit && ./scripts/build.sh && cd -` —— 或在 CI 里直接 `cd pm3_streamlit` 一步到位。
...
如果不想用 `build.sh`,直接调 `docker build` 也可以(build context 是 `pm3_streamlit/`,Dockerfile 在 `pm3_streamlit/docker/`):
...
| `streamlit run` 提示找不到模块                    | venv 没装好,或 CWD 不在 `pm3_streamlit/`         | 确认 `which python` 指向 `.venv/bin/python`,重新 `pip install -r requirements.txt` |
| `streamlit run` 找不到 `data/protein.txt`         | CWD 不在 `pm3_streamlit/`(代码用 `__file__` 反推项目根) | `cd pm3_streamlit` 后再跑 |
